...
if __name__ == "__main__":
    ...
    spark = SparkSession\
        .builder\
        .appName("NYC Rideshare Analysis Task 7")\
        .getOrCreate()
    ...
    merged_rideshare_df = merged_rideshare_df.withColumn(
        "Route",
        F.concat_ws(" to ", F.col("Pickup_Zone"), F.col("Dropoff_Zone"))
    )

    # Calculate the count of trips for each route for Uber and Lyft separately
    route_counts = merged_rideshare_df.groupBy("Route", "business") \
        .count() \
        .groupBy("Route") \
        .pivot("business") \
        .sum("count") \
        .na.fill(0)  # Replace nulls with zeros if any business doesn't have a route

    # Rename the columns to 'uber_count' and 'lyft_count'
    route_counts = route_counts.withColumnRenamed("Uber", "uber_count") \
                               .withColumnRenamed("Lyft", "lyft_count")

    # Calculate the 'total_count'
    route_counts = route_counts.withColumn(
        "total_count",
        F.col("uber_count") + F.col("lyft_count")
    )

    # Get the top 10 popular routes in terms of the total trip count
    top_10_routes = route_counts.orderBy(F.desc("total_count")).limit(10)

    # Show the top 10 routes without truncating
