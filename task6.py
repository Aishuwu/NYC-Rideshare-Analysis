...
from pyspark.sql.functions import to_date, col, from_unixtime, count, when, lit
...
if __name__ == "__main__":
    ...
    spark = SparkSession\
        .builder\
        .appName("NYC Rideshare Analysis Task 6")\
        .getOrCreate()
    ...
    # filtering data to find trip counts greater than 0 and less than 1000 for different Pickup_Borough at different time_of_day
    trip_counts = merged_rideshare_df.groupBy("Pickup_Borough", "time_of_day") \
        .count() \
    ...
    # calculating the number of trips for each Pickup_Borough in the evening
    evening_trip_counts = (merged_rideshare_df.filter(col("time_of_day") == "evening")
    ...
    print("Number of Trips for Each 'Pickup_Borough' in the Evening Time:")
    evening_trip_counts.show()

    # calculating the number of trips that started in Brooklyn and ended in Staten Island
    brooklyn_to_staten_island_trips = merged_rideshare_df.filter(
        (col("Pickup_Borough") == "Brooklyn") & (col("Dropoff_Borough") == "Staten Island")
    ).select("Pickup_Borough", "Dropoff_Borough", "Pickup_Zone")

    brooklyn_to_staten_island_trips.show(10, truncate=False)
    print("Number of Trips from Brooklyn to Staten Island:", brooklyn_to_staten_island_trips.count())
